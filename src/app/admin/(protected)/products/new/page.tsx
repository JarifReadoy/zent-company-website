...
const emptyForm = {
  ...
  icon: 'Building', features: '', benefits: '', useCases: '', featured: false,
};
    ...
    <div className="p-8 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <RichTextEditor value={form.fullDescription} onChange={v => set('fullDescription', v)} placeholder="Write a detailed product description..." />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Icon (SVG URL or Name)</label>
            <div className="flex gap-2">
              <input value={form.icon} onChange={e => set('icon', e.target.value)} className="w-full bg-gray-800 border border-gray-700 text-white px-3 py-2.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          ...
          <div className="flex items-end pb-1">
            <label className="flex items-center gap-2 text-gray-300 cursor-pointer">
              <input type="checkbox" checked={form.featured} onChange={e => set('featured', e.target.checked)} className="w-4 h-4 accent-indigo-500" />
              Featured product
            </label>
          </div>
        </div>
        <TextArea label="Features (one per line)" value={form.features} onChange={v => set('features', v)} />
        <TextArea label="Benefits (one per line)" value={form.benefits} onChange={v => set('benefits', v)} />
        <TextArea label="Use Cases (one per line)" value={form.useCases} onChange={v => set('useCases', v)} />
        <div className="flex gap-3 pt-2">
          ...
          <Link href="/admin/products" className="text-gray-400 hover:text-white px-4 py-2.5 rounded-lg transition-colors">Cancel</Link>
        </div>
      </form>
    </div>
  );
